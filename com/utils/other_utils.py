...
def get_rectangles(self):
    ...
    # Find contours
    contours, _ = cv2.findContours(gray, cv2.RETR_TREE, cv2.CHAIN_APPROX_TC89_KCOS )
    # Loop through the contours and approximate rectangles
    ...
    for contour in contours:
        ...
        # Check if the shape is a rectangle (you can customize this condition)
        if len(approx) == 4:
            self.rectangular_contours.append(approx)
            
    self.image_with_all_contours = self.image.copy()    
    cv2.drawContours(self.image_with_all_contours, self.rectangular_contours, -1, (0, 255, 0), 3)
    self.store_image(self.image_with_all_contours, 'testing.jpg')
    return self.rectangular_contours

class OCRService2(ActWithImage):
