...
class TableExtractor:
    def __init__(self, image_path="", suffix = ""):
        self.image_path = image_path
        self.suffix = suffix

    def main_execution(self):
        self.find_contours()
        self.store_process_image("7_all_contours{}.jpg".format(self.suffix), self.image_with_all_contours)
        self.filter_contours_and_leave_only_rectangles()
        self.store_process_image("8_only_rectangular_contours{}.jpg".format(self.suffix), self.image_with_only_rectangular_contours)
        self.find_largest_contour_by_area()
        self.store_process_image("9_contour_with_max_area{}.jpg".format(self.suffix), self.image_with_contour_with_max_area)
        self.order_points_in_the_contour_with_max_area()
        self.store_process_image("10_with_4_corner_points_plotted{}.jpg".format(self.suffix), self.image_with_points_plotted)
        self.calculate_new_width_and_height_of_image()
        self.apply_perspective_transform()
        self.store_process_image("11_perspective_corrected{}.jpg".format(self.suffix), self.perspective_corrected_image)
        self.add_10_percent_padding()
        self.store_process_image("12_perspective_corrected_with_padding{}.jpg".format(self.suffix), self.perspective_corrected_image_with_padding)
        return self.contour_with_max_area
    ...
    def find_contours(self):
        self.contours, self.hierarchy = cv2.findContours(self.closed_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
